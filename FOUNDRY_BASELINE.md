...
- Domain Flux helpers added under `lib/flux/` (`preferences.ts`, `custom-tunings.ts`, `favorites.ts`) — this makes `foundry:status` report `locally_customized` on `lib/flux/**` fingerprints. Expected for a domain fork; do not re-stamp.
